...
RAM_LIMIT = '1g' #Set Your Own Ram How Much You Want To Give Your Users
SERVER_LIMIT = 5 #you can change it!
...
whitelist_ids = {"1248625665696337964"}  # Replace with actual user IDs
...
async def create_server_task(interaction):
    await interaction.response.send_message(embed=discord.Embed(description="### Creating Instance, This takes a few seconds. Powered by [SRJ-HOSTING](<https://discord.gg/U23sgUHUFV>)", color=0x00ff00))
    userid = str(interaction.user.id)
    ...
    try:
        container_id = subprocess.check_output([
           "docker", "run", "-itd", "--privileged", "--hostname", "srjhosting", "--cap-add=ALL", image
        ]).strip().decode('utf-8')
    except subprocess.CalledProcessError as e:
